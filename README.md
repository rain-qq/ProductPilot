...
- ✅ **现代化前端**: React + TypeScript + TailwindCSS 可视化界面
...
- 🎨 **直观的配置表单**: 填写商品信息即可生成
- 📊 **实时进度追踪**: 可视化显示6个处理步骤
- 🖼️ **图片画廊**: 展示生成的所有图片及质量评分
- ⚡ **一键下载**: 快速保存生成的图片
- 📱 **响应式设计**: 完美适配桌面和移动设备

![Frontend Preview](./frontend-preview.png)

详细前端文档请查看: [frontend/README.md](./frontend/README.md)
...
```
┌─────────────────────────────────────┐
│     React Frontend (Port 5173)      │  ← 用户界面
└──────────────┬──────────────────────┘
               │ HTTP API
┌──────────────▼──────────────────────┐
│       FastAPI Backend (Port 8000)   │  ← API服务
└──────────────┬──────────────────────┘
               │
┌──────────────▼──────────────────────┐
...
| MINIO_BUCKET_NAME | MinIO存储桶名称 | ecommerce-images |
...
### Q: 前端无法连接后端
A: 检查 `frontend/.env` 中的 `VITE_API_BASE_URL` 是否指向正确的后端地址。

### Q: MinIO连接失败
A: 确保MinIO服务正在运行，并检查 `.env` 中的MinIO配置。
